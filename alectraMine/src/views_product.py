...
#----------prints table/ boxes of ALL PRODUCTS---------------------------------------------------------------------------------------------------- 
def productsPrinter():
...
#Creates the page of individual product   
def get_message(ID):
    command = """SELECT {products}.prod_id, {products}.prod_name, {products}.dept_id, {products}.prod_price, 
    {products}.prod_stock, {products}.prod_rating, {products}.prod_images
    FROM {products} JOIN {departments} ON {products}.dept_id = {departments}.dept_id
    WHERE {products}.prod_id = {num_id}
    """.format(products = 'products', departments = 'departments', num_id = ID)
